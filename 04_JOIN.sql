...
[JOIN 용어 정리]
----------------------------------------------------------------------------------------------------------------ㅣ
  오라클       	  	       ㅣ                         SQL : 1999표준(ANSI)                                      ㅣ
----------------------------------------------------------------------------------------------------------------ㅣ
등가 조인		           ㅣ               내부 조인(INNER JOIN), JOIN USING / ON                              ㅣ
                           ㅣ                 + 자연 조인(NATURAL JOIN, 등가 조인 방법 중 하나)		            ㅣ 
----------------------------------------------------------------------------------------------------------------ㅣ
포괄 조인 		           ㅣ             왼쪽 외부 조인(LEFT OUTER), 오른쪽 외부 조인(RIGHT OUTER)             ㅣ
                           ㅣ                 + 전체 외부 조인(FULL OUTER, 오라클 구문으로는 사용 못함)         ㅣ
----------------------------------------------------------------------------------------------------------------ㅣ
자체 조인, 비등가 조인     ㅣ	                    JOIN ON                                                     ㅣ
----------------------------------------------------------------------------------------------------------------ㅣ
카테시안(카티션) 곱		   ㅣ            교차 조인(CROSS JOIN)                                                  ㅣ
CARTESIAN PRODUCT 		   ㅣ                                                                                   ㅣ
----------------------------------------------------------------------------------------------------------------ㅣ
...
FROM EMPLOYEE 
WHERE DEPT_CODE(+)  = DEPT_ID(+);
 --> SQL Error [904] [42000]: ORA-00904: "DEPT_ID": 부적합한 식별자
...
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
  -->  Error [904] [42000]: ORA-00904: "LOCATION_ID": 부적합한 식별자
...
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE EMP_NO >= '1970-01-01' AND SUBSTR(EMP_NO, 8,1) = 2 AND EMP_NAME LIKE'전%';
...
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
--WHERE DEPT_TITLE LIKE '%해외영업1%' OR DEPT_TITLE LIKE '%해외영업2%'; -- 내 풀이
WHERE DEPT_TITLE IN('해외영업1부','해외영업2부'); -- 강사 풀이 
