...
-- 전 직원의 급여 평균보다 많은(초가) 급여를 받는 직원의 
...
-- 3-2) ANY를 이용하여 과장 중 가장 급여가 적은 직원 초과하는 대리를 조회  (복습하기)
SELECT EMP_ID , EMP_NAME , JOB_NAME, SALARY    
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
...
-- 해당 행이 서브쿼리의 조건을 충족하는지 확인하여 SELECT를 진행됨

	-- ** 해석 순서가 기존 서브쿼리와 다르게
	-- ** 메인 쿼리 1행 -> 1행에 대한 서브 쿼리						   	
	-- ** 메인 쿼리 2행 -> 2행에 대한 서브 쿼리						   	
	-- ** ....						   	 
	-- ** 메인 쿼리의 행의 수 만큼 서브 쿼리가 생성되어 진행됨						   	
	-- **						   	
...
ORDER BY SALARY DESC;
 ...
 -- >> FROM절에 작성되기 때문애 해석 1순위
 -- 2) 메인 쿼리 조회시 ROWNUM을 5이하 까지만 조회
