...
/* 자료형
    ...
    DATE : 날짜 타입
    BLOB : 대용량 이진 데이터 (4GB) - 지양되는 방식(데이터베이스에 직접적으로 영상, 사진을 저장하는 것은 좋은 방식이 아님) 
    CLOB : 대용량 문자 데이터 (4GB)
*/
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES('MEM02', 'QWER1234', '이충재', '961512-1234567', SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES('MEM03', 'QWER1562', '최진경', '941512-2234657', DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
--INSERT INTO 테이블명(컬러명1, 컬럼명2, 컬럼명3) VALUES(값1, 값2, 값3);
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME) VALUES('MEM04', 'qwe4654', '김준혁');

-- ** JDBC에서 날짜를 입력 받았을 때 삽입하는 방법 **
-- '2022-09-13 17:33:27'
INSERT INTO MEMBER VALUES('MEM05','PASS05','김태원','920303-1234567',
...
-- ** [ NUMBER 타입의 문제점 ] **
-- MEMBER2 TABLES(아이디, 비밀번호, 이름, 전화번호)
...
INSERT INTO MEMBER2 VALUES('MEM02','PASS02', '고길순', 01055551111);
...
CREATE TABLE USER_USED_NN(
    ...
    USER_ID VARCHAR2(20),        --> 컬럼 레벨 제약 조건 설정
...
-- 제약  조건 지정 방법
-- 1) 컬럼 레벨   : [CONSTRAINT 제약 조건명] 제약조건
-- 2) 테이블 레벨 : [CONSTRAINT 제약 조건명] 제약조건 (컬럼명)
...
CREATE TABLE USER_USED_UK2(
    ...
    CONSTRAINT USER_ID_NANE_U UNIQUE(USER_ID, USER_NAME)
);
...
-- 테이블에 대한 식별자(IDENTIFIER) 역할을 함 --> 식별자 : 전화번호, 주민등록번호, 학번 등을 의미 
...
CREATE TABLE USER_USED_CHECK(
  USER_NO NUMBER PRIMARY KEY,
  USER_ID VARCHAR2(20) UNIQUE,
  USER_PWD VARCHAR2(30) NOT NULL,41
  USER_NAME VARCHAR2(30),
...
COMMENT ON COLUMN USER_TEST.USER_PNO IS '주민등록번호';
...
-- 서브 쿼리로 테이블 생성 시 테이블의 형태(컬럼명, 데이터 타입)만 복사되고 
