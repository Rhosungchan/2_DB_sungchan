  ...
  ***** VIEW 사용시 주의사항 *****   
  ...
  2) VIEW를 이용한 DML(INSERT / UPDATE / DELETE)가 가능한 경우도 있지만 
    ...
    -- 4) WITH READ ONLY 옵션 : 뷰에 대해 조회만 가능(DML 수행 불가)   
 	...
 	-- OR REPLACE 옵션 / WITH READ ONLY 옵션 : 중요(나머지는 잘안씀)
 * */
...
-- SQL Error [1031] [42000]: ORA-01031: 권한이 불충분합니다
   --> 1) SYS 관리자 계정 접속
   --> 2) ALTER SESSION SET "ORACLE_SCRIPT" = TRUE;
   --> 3) GRANT CREATE VIEW TO kh_rsc;
   --> ViEW 생성 구문 다시 실행
...
--> VIEW를 이용한 INSERT 시 
-- VIEW가 아닌 원본테이블에 삽입이 진행된다.

-- VIEW에 포함되지 않은 컬럼 (DEPT_TITLE)에는 
-- INSERT 시 NULL이 삽임됨. ('D0',NULL,'L3')

-- 만약 VIEW에 포함되지 않은 컬럼(DEPT_TITLE)에
-- NOT NULL제약조건이 설정되어 있는 경우
-- 정상적인 INSERT가 진행되지 않는다.
--> ORA-01400: NULL을 ("KH_RSC"."DEPT_COPY2")
