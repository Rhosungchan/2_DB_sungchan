...
--           ADD [CONSTRAINT 제약 조건명] 제약조건 (지정할 컬러명)
...
ADD CONSTRAINT DEPT_TITLE_UU UNIQUE(DEPT_TITLE);

-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE 삭제
ALTER TABLE DEPT_COPY
DROP CONSTRAINT DEPT_TITLE_UU ;


-- *** DEPT_COPY의 DEPT_TITLE 컬럼에 NOT NULL 제약조건 추가/삭제 *** (복습하기)
ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COPT_TITLE_NN NOT NULL (DEPT_TITLE);
--> NOT NULL 제약조건은 새로운 조건을 추가하는것이 아닌
-- 컬럼 자체에 NULL허용/비허용을 제어하는 성질 변경의 형태로 인식됨
...
VALUES('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
   --> SQL Error [12899] [72000]: ORA-12899: "KH_RSC"."DEPT_COPY"."DEPT_ID" 
...
-- 3) 테이블명 변경 (DETP_COPY -> DCOPY)
ALTER TABLE DEPT_COPY RENAME TO DCOPY; --> 테이블명 변경 
SELECT * FROM DEPT_COPY; --> 오류
SELECT * FROM DCOPY;     --> 실행 성공 
...
INSERT INTO TB2 VALUES(14,4); --> 부모 값에 4 가 없으므로 오류 발생 
...
-- 3) DROP TALBE 삭제 옵션 CASCADE CONSTRAINTS 사용 
      --=> CASCADE CONSTRAINTS : 삭제하려는 테이블과 연결된 KF 제약 조건을 모두 삭제
