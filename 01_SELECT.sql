...
-- EMP_ID, EMP_NAME, PHONE 만 보고 싶은 경우
...
-- DUAL(DUmmy tAbLe(대문자 부분이 약어)) 테이블 : 실존하지 않는 가짜 테이블(임시 조회용 테이블)
...
/*
 * 방법 1.  컬럼명 AS 별칭  = 별칭 띄어쓰기X, 특수문자X, 문자만 o
 *
 * 방법 2.  컬럼명 AS "별칭"  = 별칭 띄어쓰기o, 특수문자o, 문자만 o
 * 
 * AS는 생략 가능 
 */
...
----------------------------[WHERE 사용법]----------------------------------
--                                                        연산 순위 
-- SELECT절        :     SELECT 컬럼명                        3
-- FROM절          :     FROM 테이블명                        1
-- WHERE절(조건절) :     WHERE 컬럼명 연산자 값;              2  
...
WHERE EMP_NAME LIKE '전__'; -- 이름이 3글자인 경우 가능
...
-- 부서코드가 'D9' 또는 'D6'이고                      -> AND가 OR보다 우선순위가 놓다. / ()사용 가능  
...
/* ORDER BY 절 
   - SELECT문의 조회 결과(RESULT SET)를 정렬할 떄 사용하는 구문 
   
   - (중요) SELECT문 해석 시 가장 마지막에 해석된다. 
   - [작성법]=====================================================================
  연산순위                   
      3      SELECT절 
      1      FROM절
      2      WHERE절
      4      ORDER BY 컬럼명 | 별칭 | 컬럼 순서   [ASC | DESC]   [NULLS FIRST|LAST]        -- []는 생략가능
                                                (오름/내림 차순)
     ==============================================================================
*/
