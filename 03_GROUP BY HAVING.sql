...
-- 기능 확인 예시 1) EMPLOYEE 테이블에서 부서코드, 부서별 급여 합 조회 
-- 1) 부서 코드만 조회 
...
-- 기능 확인 예시 2) EMPLOYEE 테이블에서 직급코드가 같은 사람의 
--                   직급코드, 급여 평균, 인원 수를 조회 (직급 코드 오름 차순으로 정렬)
...
-- 기능 확인 예시 3) EMPLOYEE 테이블에서 성별과 각 성별 별 인원 수, 급여 합을 
-- 인원 수 내림차순으로 조회
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남','2','여') 성별,
       COUNT(*) "인원 수",
       SUM(SALARY)"급여 합" 
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남','2','여') -- [성별] 별칭 사용 x (SELECT절 해석 x)
ORDER BY "인원 수";  -- 별칭 사용 가능 (SELECT절 해석o)
...
-- 실습 예시 1) EMPLOYEE 테이블에서 부서코드가 D5, D6인 부서의 평균 급여, 인원 수 조회
...
-- 실습 예시 2) EMPLOYEE 테이블에서 직급 별 2000년도 이후 입사자들의 급여 합을 조회
-- 풀이 1 (내가 한 것)
...
--TO_NUMBER(TO_CHAR(HIRE_DATE,'YYYY'),1,4) >='2000'
GROUP BY JOB_CODE 
ORDER BY 1;
...
-- 실습 예시 1) 부서별 평균 급여가 3백만원 이상인 부서를 조회 (부서코드 오름차순)
...
-- 실습 예시 2) EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인 직급코드, 인원수 조회(직급코드 오름차순)
...
-- [2022-09-02] ===================================================
-- [집계함수]=================================================
-- 집계함수 (ROLLUP, CUBE) = 참고용으로 알아두자! 추후 정보처리기사 시험에 등장한다. 
...
-- [CUBE] -----------------------------------------------------
-- CUBE : GROUP BY절에서 가장 먼저 작성된 모든 컬럼의 중간 집계를 처리하는 함수
SELECT DEPT_CODE ,JOB_CODE ,COUNT(*) 
FROM EMPLOYEE
...
/* SET OPERATOR(집합 연산자)
  ...
  - UNION (합 집합) : 두 SElECT 결과를 하나로 합침
                      단, 중복은 한 번만 작성 
...
-- 집합 A) 부서 코드가 'D5'인 사원의 사번, 이름, 부서코드, 급여
...
--INTERSECT -- 2행
MINUS -- 4행
-- 집합 B) 급여가 300만 초과인 사원의 사번, 이름, 부서코드, 급여 
SELECT EMP_ID , EMP_NAME  , DEPT_CODE , SALARY 
FROM EMPLOYEE 
...
SELECT EMP_ID , EMP_NAME  , DEPT_CODE , SALARY , '1'
FROM EMPLOYEE 
WHERE DEPT_CODE ='D5'
UNION
SELECT EMP_ID , EMP_NAME  , DEPT_CODE , SALARY 
