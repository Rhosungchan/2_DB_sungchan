...
-- 지정한 위치부터 지정한 순번쨰로 검색되는 문자의 위치를 반환 

-- 문자열을 앞에서 부터 검색하여 첫 번째 B위치 조회 
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; 

-- 문자열을 5번째 문자부터 검색하여 첫 번째 B위치 조회
...
-- 활용 예시 1) EMPLOYEE 테이블에서 사번이 짝수인 사원의 사번, 이름 조회 

SELECT EMP_ID , EMP_NAME 
FROM EMPLOYEE 
WHERE MOD(EMP_ID,2) = 0; 

-- 활용 예시 2) EMPLOYEE 테이블에서 사번이 홀수인 사원의 사번, 이름 조회 
...
SELECT ROUND(123.456, -1) FROM DUAL; -- 120 (소수점 0 번째 자리에서 반올림)
                                     --> 소수점 -1 번째 자리 표현
                                     -- == 1의 자리에서 반올림해서 10의 자리부터 표현

SELECT ROUND(123.456, -2) FROM DUAL; -- 100 (10의 자리에서 반올림해서 100자리부터 표현)
...
SELECT TRUNC(123.465) FROM DUAL; -- 123 / 소수점 아래 버림
SELECT TRUNC(123.465 , 1) FROM DUAL; -- 123.4 / 소수점 첫째 자리 아래 버림
SELECT TRUNC(123.465 , -1) FROM DUAL; -- 120 / 10의 자리 아래 버림
...
SELECT MONTHS_BETWEEN(SYSDATE, '2022-08-04') FROM DUAL;  -- 0.9174734916367980884109916367980884109916
SELECT ROUND( MONTHS_BETWEEN(SYSDATE, '2022-08-04'), 3) "수강기간(개월)" FROM DUAL;  -- 0.918    // 0.9개월

-- 활용 예시) EMPLOYEE 테이블에서 사원의 이름, 입사일, 근무한 개월 수, 근무 년차 조회
...
SELECT LAST_DAY('2020-02-01')FROM DUAL;  -- 해당 년도의 윤달 구분 
...
-- 활용 예시) EMPLOYEE 테이블에서 각 사원의 입사 년도, 월, 일 조회
SELECT EMP_NAME,
...
SELECT SYSDATE FROM DUAL;                                          -- 2022-09-01 11:21:16.000
SELECT TO_CHAR(SYSDATE, 'YYYY/MM/DD/ HH24:MI:SS DAY') FROM DUAL;   -- 2022/09/01 11:21:16 목요일
SELECT TO_CHAR(SYSDATE, 'MM/DD (DAY)') FROM DUAL;                  -- 09/01 (목요일)

SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" (DY)') FROM DUAL;  -- 2022년 09월 01일 (목)
	 -- [Error] ORA-01821: 날짜 형식이 부적합니다
	 -- 년, 월, 일이 날짜를 나타내는 패턴으로 인식이 안되어 오류 발생
	 -- 해결방법 : " "쌍따옴표를 이용해서 단순한 문자로 인식시키면 해결됨
...
SELECT TO_DATE(20220902) FROM DUAL;     -- DATE 타입으로 변환 

SELECT TO_DATE('220901 113255', 'YYMMDD HH24MISS') FROM DUAL;  -- 2022-09-01 11:32:55.000
	-- [Error] ORA-01861: 리터럴이 형식 문자열과 일치하지 않음
    --> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인지시킴

-- 활용 예시) EMPLOYEE 테이블에서 각 직원이 태어난 생년월일(1994년 01월 07일) 조회

SELECT EMP_NAME , TO_CHAR( TO_DATE( SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') -1), 'RRMMDD'), 'YYYY"년" MM"월" DD"일"') AS 생년월일 
FROM  EMPLOYEE; 
...
-- 기능 확인 예시) EMPLOYEE 테이블에서 보너스를 받으면 '0', 안받으면'X' 조회
...
SELECT EMP_NAME , SALARY,
	CASE
		...
		WHEN SALARY >= 3000000 THEN '소'  -- else IF
		ELSE '소'
	END "급여 수준"
FROM EMPLOYEE;
...
-- 기능 확인 예시 1) 급여 최소값, 가장 빠른 입사일, 알파벳 순서가 가장 빠른 이메일
SELECT MIN(SALARY) , MIN(HIRE_DATE) , MIN(EMAIL)  
FROM EMPLOYEE;

-- 기능 확인 예시 2) 급여 최대값, 가장 느린 입사일, 알파벳 순서가 가장 느린 이메일
SELECT MAX(SALARY) , MAX(HIRE_DATE) , MAX(EMAIL)  
FROM EMPLOYEE;

-- 활용 확인 예시 3) EMPLOYEE 테이블에서 급여를 가장 많이 받는 사원의 
--                   이름, 급여, 직급 코드 조회
...
SELECT COUNT(*) FROM EMPLOYEE; WHERE BONUS IS NOT NULL;  -- 9명
